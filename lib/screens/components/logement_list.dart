...
Future<void> _deleteLogement(String logementId, int logementIndex) async {
  try {
    if (logementIndex >= 0 && logementIndex < logements.length) {
      final response = await http.delete(Uri.parse('http://192.168.1.2:3000/api/logements/$logementId'));
...
Widget _buildYearHeader(String lieu) {
  return Container(
    padding: EdgeInsets.all(8.0),
    color: Colors.blue,
    child: Text(
      'Year ${DateTime.parse(lieu).year}', 
      style: TextStyle(
...
void _sortLogementsByYear() {
  setState(() {
    logements.sort((a, b) => DateTime.parse(b['lieu']).year.compareTo(DateTime.parse(a['lieu']).year));
  });
}
